...
export default class MELF {
  ...
  _updatePins () {
    ...
    const height = parseFloat(this.metadata['dimensions']['maximum_height2']) || parseFloat(this.metadata['dimensions']['max_body_length_l'])
    ...
    for (let i = 0; i < nrOfPins; i++) {
      ...
      box.position.x = ((i % 2 === 0) ? 1 : -1) * ((height) / 2)
  ...
  _createLayer1 () {
    ...
    const width = parseFloat(this.metadata['dimensions']['max_bandwidth_t'])
    const length = parseFloat(this.metadata['dimensions']['max_body_width_w'])
