...
export default class PQFN {
  ...
  _updatePins () {
    ...
    this.metadata['dimensions']['pinNumber'] = totalPins
    ...
    for (let i = 0; i < totalPins; i++) {
      ...
      if (parseInt(this.metadata['package_pin']['e_number_pins']) === 0 || parseInt(this.metadata['package_pin']['d_number_pins']) === 0) {
        if (i === 0 || (i === totalPins / 2)) {
          side++
      ...
      if (posArray[i] === 'd_number_pins') {
        box.position.x = ((side === 0) ? -1 : 1) * (width - boxbbInfo.z - parseFloat(this.metadata['package_pin']['max_lead_pullback']))
        ...
        box.position.z = ((side === 1) ? -1 : 1) * (depth - boxbbInfo.z - parseFloat(this.metadata['package_pin']['max_lead_pullback']))
  ...
  _getElectronicData () {
    ...
    const diff = new BABYLON.Vector3(parseFloat(this.metadata['silkscreen']['LineWidth']), 0, parseFloat(this.metadata['silkscreen']['LineWidth']))
