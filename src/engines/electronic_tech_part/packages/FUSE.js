...
export default class FUSE {
  ...
  _updateBody () {
    ...
    const size = parseFloat(this.metadata['dimensions']['max_body_width_w'])
    ...
    this.name.position.y = parseFloat(this.metadata['dimensions']['max_body_length_l']) / 2 + 0.01
    this.name.rotation.y = 0
    this.name.rotation.z = 0
  }
  ...
  _updatePins () {
    ...
    const height = parseFloat(this.metadata['dimensions']['maximum_height2']) || parseFloat(this.metadata['dimensions']['max_body_length_l']) / 2
    //const depth = parseFloat(this.metadata['dimensions']['max_body_width_w']) / 2
    ...
    for (let i = 0; i < totalPins; i++) {
      ...
      box.position.y = (-width - 1.8 / 2 + 0.1) // 0.1 - offset
      box.position.x = ((i % 2 === 0) ? -1 : 1) * (height * 0.95)
  ...
  _createLayer1 () {
    ...
    const width = parseFloat(this.metadata['dimensions']['max_bandwidth_t'])
  ...
  _addFsize () {
    ...
    const height = parseFloat(this.metadata['dimensions']['max_body_width_w'])
  ...
  _createBody3D () {
    var options = {
      diameterTop: parseFloat(this.metadata['dimensions']['max_body_width_w']),
      diameterBottom: parseFloat(this.metadata['dimensions']['max_body_width_w']),
      height: parseFloat(this.metadata['dimensions']['max_body_length_l']),
      tessellation: 32,
    }

    var box = BABYLON.MeshBuilder.CreateCylinder('body', options, this.scene)
  ...
  _getPin () {
    const pinLength = 1.8
    // const length = (parseFloat(this.metadata['dimensions']['pitch_e']) - parseFloat(this.metadata['dimensions']['max_body_length_l'])) / 2

    const length = parseFloat(this.metadata['dimensions']['max_bandwidth_t'])
    const height = pinLength
