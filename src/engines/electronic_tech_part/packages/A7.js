...
export default class A7 {
  ...
  _updatePins () {
    ...
    const nrOfPins2 = 2 * parseInt(this.metadata['package_pin']['number_pins_r'])
    let box
    for (let i = 0; i < nrOfPins1; i++) {
      ...
      if (i < k) {
        box.position.x = -(width / 2 - boxbbInfo.z)
    ...
    for (let i = 0; i < nrOfPins2; i++) {
      ...
      if (i < k) {
        box.position.z = -(depth / 2 - boxbbInfo.z)
