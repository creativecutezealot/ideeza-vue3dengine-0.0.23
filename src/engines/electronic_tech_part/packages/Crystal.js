...
export default class Crystal {
  ...
  _updateBody () {
    ...
    this.name.position.y = height * 0.8 / 2 + 0.01

    if (this.drill) {
      this.drill.dispose(false, true)
    }

    // this.drill = BABYLON.MeshBuilder.CreateCylinder('drill', { diameter: depth * 0.9, height: 0.5}, this.scene)
    this.drill = new BABYLON.Mesh.CreatePlane('drill', 1, this.scene)
    this.drill.setParent(this.body)
    this.drill.setEnabled(false)
    this.drill.position.y = height * 0.8 / 2
  ...
  _updatePins () {
    ...
    // const depth = parseFloat(this.metadata['dimensions']['d_max_body_span']) / 2 // e_max_body_span
    ...
    for (let i = 0; i < xLength; i++) {
      ...
      const offsetX = parseFloat(this.metadata['package_pin']['e1_pitch']) * ((parseInt(this.metadata['package_pin']['e_number_pins']) - 1) / 2)
      // const offsetY = parseFloat(this.metadata['package_pin']['e_pitch']) / 2 * (parseInt(this.metadata['package_pin']['d_number_pins']) - 1)
      
      if (i < 2) {
        // with 2 pins
        box.position.x = 0
        box.position.z = i * parseFloat(this.metadata['package_pin']['e1_pitch']) - offsetX
        // original version wit h3 pins
        // box.position.x = parseFloat(this.metadata['package_pin']['e_pitch']) - offsetY
        // box.position.z = i * parseFloat(this.metadata['package_pin']['e1_pitch']) - offsetX / 2
      }
      else {
        box.position.x = -1 * parseFloat(this.metadata['package_pin']['e_pitch']) / 2 - 0.3
        box.position.z = 0 * parseFloat(this.metadata['package_pin']['e1_pitch']) - offsetX / 2
      }
  ...
  _createBody3D () {
    const length = (parseFloat(this.metadata['dimensions']['e_max_body_span']) > parseFloat(this.metadata['dimensions']['d_max_body_span'])) ? parseFloat(this.metadata['dimensions']['e_max_body_span']) : parseFloat(this.metadata['dimensions']['d_max_body_span'])

    var options1 = {
      diameterTop: length, // * 0.9, // original version * 0.9
      diameterBottom: length, // * 0.9, // original version * 0.9
      height: parseFloat(this.metadata['dimensions']['maximum_height']) * 0.8,
      tessellation: 32
    }

    const box1 = BABYLON.MeshBuilder.CreateCylinder('body', options1, this.scene)
 
    var options2 = {
      diameterTop: length,
      diameterBottom: length,
      height: parseFloat(this.metadata['dimensions']['maximum_height']) * 0.2,
      tessellation: 32
    }

    const box2 = BABYLON.MeshBuilder.CreateCylinder('body', options2, this.scene)
    box2.position.y = -parseFloat(this.metadata['dimensions']['maximum_height']) / 2 + parseFloat(this.metadata['dimensions']['maximum_height']) * 0.2 / 2

    var innerCSG = BABYLON.CSG.FromMesh(box1)
    box1.dispose()
