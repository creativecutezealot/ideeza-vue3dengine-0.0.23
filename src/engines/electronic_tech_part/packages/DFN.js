...
export default function (scene, metadata) {
  ...
  this._initDFN = function () {
    //  add Marian - need to come from formDFN
    ...
    this.object3D = this._createDFN3D()
  ...
  // CREATE 3D DFN OBJECT
  this._createDFN3D = function () {
    ...
    this._addPinsDFNComp(box)

    box.material = bodymaterial

    this._createDFN2D(box)

    // create an empty mesh to hold the entire components
    var mesh = new BABYLON.Mesh('mesh', this.scene)
    mesh.name = 'DFN' // this.metadata['name'] + this.metadata['part'] + this.metadata['id']
    box.setParent(mesh)

    return mesh
  }

  //  ADD PINS DFN COMPONENT
  this._addPinsDFNComp = function (comp) {
    comp.pins = []

    let totalPins = 0
    if (parseInt(this.metadata['dimensions']['packageType']) === 2) {
      totalPins = 4
    }
    else {
      if (parseInt(this.metadata['dimensions']['packageType']) === 1) {
        totalPins = 3
      }
      else {
        totalPins = 2
    ...
    for (let i = 0; i < totalPins; i++) {
      switch (totalPins) {
        case 2:
          ...
          box.position.z = 0
          if (i === 0) {
            box.position.x = parseFloat(this.metadata['package_pin']['e2_pitch'])
          }
          else {
            box.position.x = parseFloat(this.metadata['package_pin']['e2_pitch']) - parseFloat(this.metadata['package_pin']['e1_pitch'])
          }
        ...
        case 3:
          ...
          if (i === 0) {
            ...
            box.position.z = ((i % 2 === 0) ? -1 : 1) * parseFloat(this.metadata['package_pin']['e3_pitch']) / 2
            box.position.x = parseFloat(this.metadata['package_pin']['e2_pitch']) - parseFloat(this.metadata['package_pin']['e1_pitch'])
          }
  ...
  //  CREATE 2D DFN OBJECT
  this._createDFN2D = function (comp) {
    let meshes = []
  ...
  this._getPin = function (pinType) {
    let width
    let length
    if (pinType === 1) {
      width = parseFloat(this.metadata['package_pin']['l2_body_width_max'])
      length = parseFloat(this.metadata['package_pin']['b2_body_width_max'])
    }
    else {
      width = parseFloat(this.metadata['package_pin']['l1_body_width_max'])
      length = parseFloat(this.metadata['package_pin']['b1_body_width_max'])
    }
  ...
  this.sendData = function () {
    ...
    let diff
    if (parseInt(this.metadata['dimensions']['packageType']) === 2) {
      diff = new BABYLON.Vector3(parseFloat(Number(pins[pins.length - 2].getAbsolutePosition().x + x - pins[pins.length - 2].metadata.Fwidth / 2).toFixed(4)), 0, parseFloat(Number(pins[pins.length - 2].getAbsolutePosition().z + z - pins[pins.length - 2].metadata.Flength / 2).toFixed(4)))
    }
    else {
      diff = new BABYLON.Vector3(parseFloat(Number(pins[pins.length - 1].getAbsolutePosition().x + x - pins[pins.length - 1].metadata.Fwidth / 2).toFixed(4)), 0, parseFloat(Number(pins[pins.length - 1].getAbsolutePosition().z + z - pins[pins.length - 1].metadata.Flength / 2).toFixed(4)))
    }
