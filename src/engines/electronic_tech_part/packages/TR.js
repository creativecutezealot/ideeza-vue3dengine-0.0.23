...
export default class TR {
  ...
  _updateBody () { 
    ...
    // w h d
    const width = parseFloat(this.metadata['dimensions']['max_body_width_e1'])
    const height = parseFloat(this.metadata['dimensions']['maximum_height'])
    const depth = parseFloat(this.metadata['dimensions']['max_body_length_d'])
  ...
  _updatePins () {
    ...
    const width = parseFloat(this.metadata['dimensions']['max_body_width_w']) / 2
    ...
    for (let i = 0; i < totalPins; i++) {
      ...
      box.position.y = -(parseFloat(this.metadata['dimensions']['maximum_height']) + boxbbInfo.y) / 2

      box.position.z = 0
      const diff = (parseFloat(this.metadata['dimensions']['max_body_width_d']) - parseFloat(this.metadata['dimensions']['max_body_width_w'])) / 2
      box.position.x = ((i % 2 === 0) ? -1 : 1) * (width - boxbbInfo.x + diff)
  ...
  _addFSize () {
    ...
    const height = parseFloat(this.metadata['dimensions']['maximum_height']) + 0.2 // 0.2 - height of pin
  ...
  _createBody3D () {
    ...
    var cylinder = BABYLON.MeshBuilder.CreateBox('body', options, this.scene)
    cylinder.position.y = -(parseFloat(this.metadata['dimensions']['max_body_height']) / 2 - parseFloat(this.metadata['dimensions']['maximum_height']) / 2)

    const width = parseFloat(this.metadata['dimensions']['max_body_width_w']) - 1
    const depth = parseFloat(this.metadata['dimensions']['max_body_length_l']) - 1
    const height = parseFloat(this.metadata['dimensions']['max_body_height'])
    ...
    const sphere = BABYLON.MeshBuilder.CreateCylinder('cylinder1', options2, this.scene)
    const sphere2 = sphere.clone('cylinder2')

    sphere.position.x = width / 2 - depth / 2
    sphere2.position.x = -(width / 2 - depth / 2)
    // sphere.position.y = sphere2.position.y = height / 2
    ...
    var cylinder2 = BABYLON.MeshBuilder.CreateBox('body', options3, this.scene)
    ...
    sphere2.dispose()
    var innerCSG4 = BABYLON.CSG.FromMesh(cylinder2)
    cylinder2.dispose()

    var subCSG = innerCSG.union(innerCSG2).union(innerCSG3).union(innerCSG4)
    ...
    const box = subCSG.toMesh('body', null, this.scene)
