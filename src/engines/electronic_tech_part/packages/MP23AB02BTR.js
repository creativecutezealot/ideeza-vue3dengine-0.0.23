...
export default function (scene, metadata) {
  ...
  this._initMP23AB02BTR = function () {
    //  add Marian - need to come from formMP23AB02BTR
    ...
    this.object3D = this._createMP23AB02BTR3D()
  ...
  // CREATE 3D MP23AB02BTR OBJECT
  this._createMP23AB02BTR3D = function () {
    ...
    this._addPinsMP23AB02BTRComp(box)

    box.material = bodymaterial

    this._createMP23AB02BTR2D(box)

    // create an empty mesh to hold the entire components
    var mesh = new BABYLON.Mesh('mesh', this.scene)
    mesh.name = 'MP23AB02BTR' // this.metadata['name'] + this.metadata['part'] + this.metadata['id']
    box.setParent(mesh)
  ...
  this.createBody3D = function () {
    ...
    var options = {
      width: parseFloat(this.metadata['dimensions']['max_body_width_w2']),
      height: parseFloat(this.metadata['dimensions']['maximum_height']),
      depth: parseFloat(this.metadata['dimensions']['max_body_length_l2']),
      faceUV: faceUV
    }
    ...
    var options2 = {
      width: parseFloat(this.metadata['dimensions']['max_body_width_w']),
      height: 0.2,
      depth: parseFloat(this.metadata['dimensions']['max_body_length_l']),
      faceUV: faceUV2
    }

    const box2 = BABYLON.MeshBuilder.CreateBox('body', options2, this.scene)
    box2.position.y = -parseFloat(this.metadata['dimensions']['maximum_height']) / 2 + 0.1
    ...
    return subCSG.toMesh('body', null, this.scene)
  }

  //  ADD PINS MP23AB02BTR COMPONENT
  this._addPinsMP23AB02BTRComp = function (comp) {
    comp.pins = []
    ...
    for (let i = 0; i < totalPins; i++) {
      ...
      box.position.z = 0.935
      box.position.x = i * parseFloat(this.metadata['dimensions']['e_pitch']) - parseFloat(this.metadata['dimensions']['e_pitch']) / 2

      box.material = clone.material.clone('pinMaterial_' + i)
      comp.pins.push(box)

      if (i === 0) {
        this.startedPin.push(box)
      }
    }

    box = BABYLON.MeshBuilder.CreateCylinder('cylinder1', { diameterTop: parseFloat(this.metadata['dimensions']['pin_diameter']), diameterBottom: parseFloat(this.metadata['dimensions']['pin_diameter']), height: 0.1, tessellation: 64 }, this.scene)
    box.metadata = {
      'width': parseFloat(this.metadata['dimensions']['pin_diameter']),
      'length': parseFloat(this.metadata['dimensions']['pin_diameter'])
    }
    
    boxbbInfo = box.getBoundingInfo().boundingBox.extendSize
    box.position.y -= (bbInfo.y + boxbbInfo.y)

    box.position.z = parseFloat(this.metadata['dimensions']['pin_position'])
    box.position.x = 0

    box.material = new BABYLON.StandardMaterial('pinMaterial_' + totalPins, this.scene)
    comp.pins.push(box)
  ...
  //  CREATE 2D MP23AB02BTR OBJECT
  this._createMP23AB02BTR2D = function (comp) {
    let meshes = []
  ...
  this.sendData = function () {
    ...
    let diff = new BABYLON.Vector3(parseFloat(Number(pins[0].getAbsolutePosition().x + x - pins[0].metadata.Fwidth / 2).toFixed(4)), 0, parseFloat(Number(pins[2].getAbsolutePosition().z + z - pins[2].metadata.Flength / 2).toFixed(4)))
    if (this.startedPin.length === 2) {
      diff = new BABYLON.Vector3(parseFloat(Number(this.startedPin[0].getAbsolutePosition().x + x - this.startedPin[0].metadata.Fwidth / 2).toFixed(4)), 0, parseFloat(Number(this.startedPin[1].getAbsolutePosition().z + z - this.startedPin[1].metadata.Flength / 2).toFixed(4)))
    }
