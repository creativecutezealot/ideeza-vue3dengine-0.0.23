...
export default class LED {
  ...
  async _load3dObjects () {
    ////// body ////////
    const body = await Component.import3dObject('user-65bbd2cbeeb64ca29309a3dda956413c.glb', this.scene)
    if (!body) {
  ...
  _addFsize () {
    ...
    if (parseInt(this.metadata['package_pin']['type']) === 0) {
      ...
      this.layer1.position.y = -newPos + parseFloat(this.metadata['dimensions']['maximum_height']) - 0.3 // to check
      this.layer21.position.y = -newPos + parseFloat(this.metadata['dimensions']['maximum_height']) - 0.3 // to check
