  ...
  _updatePins () {
    ...
    for (let i = 0; i < xLength; i++) {
      for (let j = 0; j < zLength; j++) {
        ...
        box.position.x = -width + parseFloat(this.metadata['package_pin']['position']) // i * parseFloat(this.metadata['package_pin']['e1_pitch']) - offsetX
        box.position.z = j * parseFloat(this.metadata['package_pin']['e1_pitch']) - offsetY

        if (i === 1 || j === 1) {
          box.position.x += parseFloat(this.metadata['package_pin']['e_pitch'])
        }
