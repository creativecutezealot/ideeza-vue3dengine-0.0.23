...
export default class SMA {
  ...
  _updatePins () {
    ...
    pin.material = Component.createMaterial(this.scene, 'metalic', '#f5f5f5')
    pin.material.sideOrientation = 1

    // add action manager for hover
    const _this = this
    pin.actionManager = new BABYLON.ActionManager(this.scene)
    pin.actionManager.registerAction(new BABYLON.ExecuteCodeAction(BABYLON.ActionManager.OnPointerOverTrigger, (evt) => {
      Component.togglePinIndex(evt.source, true)
    }))
    pin.actionManager.registerAction(new BABYLON.ExecuteCodeAction(BABYLON.ActionManager.OnPointerOutTrigger, (evt) => {
      Component.togglePinIndex(evt.source, false)
    }))

    // show menu for material
    pin.actionManager.registerAction(new BABYLON.ExecuteCodeAction(BABYLON.ActionManager.OnRightPickTrigger, (evt) => {
      _this.selected = evt.source
      _this.tooltip.isVisible = !_this.tooltip.isVisible
    }))
  ...
  _createBody3D () {
    ...
    var options2 = {
      diameterTop: diameter / 2,
      diameterBottom: diameter / 2,
      height: height,
      tessellation: 32
    }
    var cilinder = BABYLON.MeshBuilder.CreateCylinder('body2', options2, this.scene)

    cilinder.rotation.z = Math.PI / 2
    cilinder.position.x = parseFloat(this.metadata['dimensions']['e_max_body_span']) / 2 + height / 2
    // this.metadata['externalObject']['position'] = cilinder.position

    var innerCSG = BABYLON.CSG.FromMesh(box)
    box.dispose()
    var innerCSG2 = BABYLON.CSG.FromMesh(cilinder)
    cilinder.dispose()
