...
export default class VOL {
  ...
  _updateBody () {
    ...
    body.material.transparencyMode = 2
    body.material.alpha = 0.05
    this.body = body
    ...
    this.name.position.y = 0.01
    this.name.position.z = 0.4
  ...
  _createBody3D () {
    var options = {
      ...
      height: 0.001, //parseFloat(this.metadata['dimensions']['maximum_height']),
    ...
    for (let i = 0; i < parseInt(this.metadata['package_pin']['e_number_pins']); i++) {
      for (let j = parseInt(this.metadata['package_pin']['d_number_pins']) - 1; j >= 0; j--) {
        let offsetX = 0
        ...
        const box4 = BABYLON.Mesh.CreateCylinder('pin', 10, parseFloat(this.metadata['package_pin']['max_lead_range']), parseFloat(this.metadata['package_pin']['max_lead_range']), 32, 1, this.scene, false)
        box4.position.x = i * parseFloat(this.metadata['package_pin']['e1_pitch']) - offsetX
        box4.position.z = j * parseFloat(this.metadata['package_pin']['e_pitch']) - offsetY

        box = Component._createHoles(box, box4, this.scene)
      }
    }
  ...
  _getPin () {
    // converted to only pads package - 22.08.2019
    ...
    const box = new BABYLON.AbstractMesh('pin', this.scene)
    box.metadata = {
      'width': length,
      'length': length
    }
