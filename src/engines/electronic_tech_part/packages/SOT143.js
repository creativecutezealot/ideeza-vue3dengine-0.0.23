...
export default class SOT143 {
  ...
  _updatePins () {
    ...
    for (let i = 0; i < totalPins; i++) {
      if (this.metadata['dimensions']['topLeftlargePin'] === false) {
        if (i !== parseInt(this.metadata['dimensions']['largePin'])) {
      ...
      if (i < 2) {
        ...
        let diff = parseFloat(this.metadata['package_pin']['pitch_e1'])
        if (this.metadata['dimensions']['topLeftlargePin'] === false) {
          if (parseInt(this.metadata['dimensions']['largePin']) > 1) {
            diff = parseFloat(this.metadata['package_pin']['pitch_e'])
          }
        }
        box.position.z = diff / 2 - (i % totalPins) * diff
      }
      else {
        box.position.x = width + boxbbInfo.z
        ...
        let diff = parseFloat(this.metadata['package_pin']['pitch_e1'])
        if (this.metadata['dimensions']['topLeftlargePin'] === false) {
          if (parseInt(this.metadata['dimensions']['largePin']) < 2) {
            diff = parseFloat(this.metadata['package_pin']['pitch_e'])
          }
        }
        box.position.z = diff / 2 - ((i -2) % totalPins) * diff
      }
  ...
  _createLayer1 () {
    ...
    const length = parseFloat(this.metadata['package_pin']['lead_length_l_max'])

    for (let i = 0; i < totalPins; i++) {
      if (i === 0) {
        width = parseFloat(this.metadata['package_pin']['lead_width_b2_max'])
      }
      else {
        width = parseFloat(this.metadata['package_pin']['lead_width_b_max'])
      }
