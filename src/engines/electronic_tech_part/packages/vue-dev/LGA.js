...
export default function (scene, metadata) {
  ...
  this._initLGA = function () {
    ...
    */
    ...
    this.object3D = this._createdLGA3D(this.metadata)
  ...
  //
  this._createdLGA3D = function (payload) {
    Component.dispose(this)
    console.log(`---> updateObject_m`, payload)

    let vm = {}
    vm.lga = payload // metadata earlier

    console.log('LGA METADATA=>', vm.lga)

    let depth = parseFloat(vm.lga.box.depth)
    let height = parseFloat(vm.lga.box.height)
    let width = parseFloat(vm.lga.box.width)
    let row = parseFloat(vm.lga.pin.row)
    let column = parseFloat(vm.lga.pin.column)
    // let pitch = Math.abs(parseFloat(vm.bga.pin.pitch)- parseFloat(vm.bga.pin.diameter))
    let pitch = parseFloat(vm.lga.pin.pitch)
    let diameter = parseFloat(vm.lga.pin.diameter)
...
// x
    ...
    */
    ...
    this._createLGA2D(box)

    function createPin() {
      let d = vm.lga.pin.diameter
  ...
  */
  ...
  this._createLGA2D = function (comp) {
    let meshes = []
