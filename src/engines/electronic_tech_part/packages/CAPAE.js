...
export default function (scene, metadata) {
  ...
  this._initCAPAE = function () {
    ...
    this.object3D = this._createCAPAE3D()
  ...
  this._createCAPAE3D = function () {
    ...
    this._addPinsCAPAEComp(box)

    box.material = bodymaterial

    this._createCAPAE2D(box)

    // create an empty mesh to hold the entire components
    var mesh = new BABYLON.Mesh('mesh', this.scene)
    mesh.name = 'CAPAE' // this.metadata['name'] + this.metadata['part'] + this.metadata['id']
    box.setParent(mesh)
  ...
  this.createBody3D = function () {
    ...
    var options2 = {
      diameterTop: parseFloat(this.metadata['dimensions']['diameter_max']),
      diameterBottom: parseFloat(this.metadata['dimensions']['diameter_max']),
      height: parseFloat(this.metadata['dimensions']['maxHeight']) * 0.7,
      tessellation: 32,
      faceUV: faceUV2
    }
    ...
    var options = {
      width: parseFloat(this.metadata['dimensions']['max_body_width']),
      height: parseFloat(this.metadata['dimensions']['maxHeight']) * 0.3,
      depth: parseFloat(this.metadata['dimensions']['max_body_length']),
      faceUV: faceUV
    }

    let cylinder = BABYLON.MeshBuilder.CreateBox('box', options, this.scene)
    cylinder = this._addCornersComp(cylinder)
    cylinder.position.y = -parseFloat(this.metadata['dimensions']['maxHeight']) / 2
  ...
  this._addCornersComp = function (comp) {
    ...
    for (let i = 0; i < vectors.length; i += 2) {
      var mesh = BABYLON.MeshBuilder.CreateBox('box', options, this.scene)
      mesh.position = vectors[i]
      mesh.rotation.y = Math.PI / 4
      mesh.scaling.y = parseFloat(this.metadata['dimensions']['maxHeight']) * 5
      csg.push(mesh)
    }
  ...
  //  ADD PINS CAPAE COMPONENT
  this._addPinsCAPAEComp = function (comp) {
    comp.pins = []
    ...
    for (let i = 0; i < totalPins; i++) {
      ...
      box.position.y -= (bbInfo.y + boxbbInfo.y + parseFloat(this.metadata['dimensions']['maxHeight']) * 0.3 / 2)
  ...
  //  CREATE 2D CAPAE OBJECT
  this._createCAPAE2D = function (comp) {
    let meshes = []
  ...
  this._getPin = function () {
    const width = parseFloat(this.metadata['dimensions']['lead_length_max'])
    const length = parseFloat(this.metadata['dimensions']['lead_width_max'])
