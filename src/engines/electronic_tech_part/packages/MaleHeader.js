...
export default class MaleHeader {
  ...
  async _load3dObjects () {
    ////// body ////////
    const body = await Component.import3dObject('user-d64e89c18cda4e3a9f5d8f697bbfa09e.glb', this.scene)
    if (!body) {
  ...
  _updateBody () {
    ...
    if (meshes.length !== 0) {
      const body = BABYLON.Mesh.MergeMeshes(meshes, true)
    ...
    const size = (xLength * e > height) ? height : xLength * e
    ...
    this.name.rotation.x = -Math.PI / 2
    this.name.position.z = -(zLength * e1 / 2 + 0.01)
  ...
  _updatePins () {
    ...
    const clone = this.buildPinNormal(this.assets[1])
    const xLength = parseInt(this.metadata['package_pin']['e_number_pins'])
    const zLength = parseInt(this.metadata['package_pin']['d_number_pins'])
    // const height = parseInt(this.metadata['dimensions']['maximum_height'])
  ...
  buildPinNormal (asset) {
    const pinX = parseFloat(this.metadata['package_pin']['max_lead_width'])
    const pinY1 = parseFloat(this.metadata['package_pin']['bottom_max_lead_height'])
    const pinY2 = parseFloat(this.metadata['package_pin']['top_max_lead_height'])
    const pinZ = parseFloat(this.metadata['package_pin']['max_lead_length'])
    const height = parseInt(this.metadata['dimensions']['maximum_height'])

    const p1 = asset.clone('p1')
    const p2 = asset.clone('p2')
    ...
    p1.position.y =  -(p1.scaling.z / 2 + height / 2 - 0.2 * height - 0.1)
    p2.position.y =  p2.scaling.z / 2 + height / 2 - 0.1

    return BABYLON.Mesh.MergeMeshes([p1, p2], true)
  }
  ...
  buildPin (index) {
    ...
    const pinY2 = parseFloat(this.metadata['package_pin']['top_max_lead_height'])
    const pinZ = parseFloat(this.metadata['package_pin']['max_lead_length'])
    const pinY3 = parseFloat(this.metadata['package_pin']['max_lead_height90'])
    const zLength = parseInt(this.metadata['package_pin']['d_number_pins'])
    const height = parseInt(this.metadata['dimensions']['maximum_height'])

    const p0 = this.assets[1].clone('randomp0')
    ...
    p0.position.y = pinY2 / 2 + height / 2 - 0.1

    p3.isVisible = true
    p3.scaling.x = pinY1 + (zLength - 1 - index) * parseFloat(this.metadata['package_pin']['e1_pitch'])
    p3.scaling.y = pinX
    p3.scaling.z = pinZ
    p3.rotation.z = Math.PI / 2
    p3.position.y = -(p3.scaling.x / 2 + height / 2 - 0.2 * height - 0.1)
    ...
    p2.position.y = p3.position.y - p3.scaling.x / 2
    ...
    p1.scaling.x = pinZ
    p1.scaling.y = pinX
    p1.scaling.z = pinY3 + (zLength - 1 - index) * parseFloat(this.metadata['package_pin']['e1_pitch'])
    ...
    return BABYLON.Mesh.MergeMeshes([p0, p1, p2, p3], true)
  }
  ...
  _addFsize () {
    ...
    if (parseInt(this.metadata['package_pin']['type']) === 0) {
      this.layer1.position.y = -height / 2 + 0.01
      this.layer21.position.y = -height / 2 + 0.01
      ...
      this.layer1.position.z = dist / 2 - 0.01
      this.layer21.position.z = dist / 2 - 0.01

      this.layer1.rotation.x = Math.PI / 2
      this.layer21.rotation.x = Math.PI / 2

      const newPos = parseFloat(this.metadata['package_pin']['bottom_max_lead_height']) - 0.2 * parseFloat(this.metadata['dimensions']['maximum_height']) + parseFloat(this.metadata['dimensions']['maximum_height']) / 2 - 0.1 + (parseInt(this.metadata['package_pin']['d_number_pins']) - 1) * parseFloat(this.metadata['package_pin']['e1_pitch']) / 2
      this.body.position.y = newPos
      for (let i = 0; i < this.pins.length; i++) {
        this.pins[i].position.y = newPos
      }
    }
