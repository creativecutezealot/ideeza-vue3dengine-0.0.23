...
export default function (scene, metadata) {
  ...
  this._initPLCC = function () {
    //  add Marian - need to come from formPLCC
    ...
    this.object3D = this._createPLCC3D()
  ...
  //  CREATE 3D PLCC OBJECT
  this._createPLCC3D = function () {
    ...
    var options = {
      width: parseFloat(this.metadata['dimensions']['e1_max_body_length']),
      height: parseFloat(this.metadata['dimensions']['maximum_height']) - parseFloat(this.metadata['dimensions']['min_standoff']),
      depth: parseFloat(this.metadata['dimensions']['d1_max_body_length']),
      faceUV: faceUV
    }

    var box = BABYLON.MeshBuilder.CreateBox('body', options, this.scene)

    this._addPinsPLCCComp(box)

    box.material = bodymaterial

    this._createPLCC2D(box)

    // create an empty mesh to hold the entire components
    var mesh = new BABYLON.Mesh('mesh', this.scene)
    mesh.name = 'PLCC' // this.metadata['name'] + this.metadata['part'] + this.metadata['id']
    box.setParent(mesh)

    return mesh
  }

  //  ADD PINS PLCC COMPONENT
  this._addPinsPLCCComp = function (comp) {
    comp.pins = []
   
    var totalPins = parseInt(this.metadata['package_pin']['e_number_pins']) + parseInt(this.metadata['package_pin']['d_number_pins'])

    var posArray = []
    for (var i = 0; i < totalPins; i++) {
      if (i < parseInt(this.metadata['package_pin']['d_number_pins']) / 2) {
        posArray.push('d_number_pins')
      }
      else if (i < (parseInt(this.metadata['package_pin']['e_number_pins']) / 2 + parseInt(this.metadata['package_pin']['d_number_pins']) / 2)) {
    ...
    for (let i = 0; i < totalPins; i++) {
      ...
      const diff1 = (parseFloat(this.metadata['package_pin']['d_max_lead_span']) - parseFloat(this.metadata['dimensions']['d1_max_body_length'])) / 2
      const diff2 = (parseFloat(this.metadata['package_pin']['e_max_lead_span']) - parseFloat(this.metadata['dimensions']['e1_max_body_length'])) / 2
      if (posArray[i] === 'd_number_pins') {
        box.position.x = ((side === 0) ? -1 : 1) * (bbInfo.x - boxbbInfo.z + diff1)
        box.position.z = ((side === 0) ? 1 : -1) * (k * parseFloat(this.metadata['package_pin']['e_pitch']) - parseFloat(this.metadata['package_pin']['d_max_lead_span']) / 4 - ((parseInt(this.metadata['package_pin']['d_number_pins']) % 2 === 0) ? parseFloat(this.metadata['package_pin']['e_pitch']) / 2 : 0))
        box.rotation.y = ((side === 0) ? -1 : 1) * Math.PI / 2
      }
      else {
        box.position.z = ((side === 1) ? -1 : 1) * (bbInfo.z - boxbbInfo.z + diff2)
        box.position.x = ((side === 1) ? -1 : 1) * (k * parseFloat(this.metadata['package_pin']['e_pitch']) - parseFloat(this.metadata['package_pin']['e_max_lead_span']) / 4 - ((parseInt(this.metadata['package_pin']['e_number_pins']) % 2 === 0) ? parseFloat(this.metadata['package_pin']['e_pitch']) / 2 : 0))
      }
  ...
  //  CREATE 2D PLCC OBJECT
  this._createPLCC2D = function (comp) {
    let meshes = []
    ...
    // console.log('x: ', parseFloat(this.metadata['dimensions']['e1_max_body_length']), parseInt(this.metadata['package_pin']['e_number_pins']), parseFloat(this.metadata['package_pin']['e_pitch']))
    // console.log('z: ', parseFloat(this.metadata['dimensions']['d1_max_body_length']), parseInt(this.metadata['package_pin']['d_number_pins']), parseFloat(this.metadata['package_pin']['e_pitch']))
  }
