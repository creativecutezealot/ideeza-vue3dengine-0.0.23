...
export default class Relay {
  constructor(scene, metadata) {
    ...
    this.startedPin1 = null
    this.startedPin2 = null
    this.path2d = null
  ...
  _updatePins () {
    ...
    box.position.x = parseFloat(this.metadata['dimensions']['first_pin_pos'])
  ...
  _getElectronicData () {
    ...
    if (this.startedPin1 && this.startedPin2) {
      diff = new BABYLON.Vector3(parseFloat(Number(this.startedPin1.getAbsolutePosition().x + x - this.startedPin1.metadata.Flength / 2).toFixed(4)), 0, parseFloat(Number(this.startedPin2.getAbsolutePosition().z + z - this.startedPin2.metadata.Fwidth / 2).toFixed(4)))
    }
  ...
  _sortPins () {
    ...
    if (this.metadata.isClockwise) {
      // sortPins = data
      sortPins.push(this.pins[1], this.pins[2], this.pins[0], this.pins[4], this.pins[3])
