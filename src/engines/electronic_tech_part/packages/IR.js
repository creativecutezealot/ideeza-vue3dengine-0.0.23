...
export default class IR {
  ...
  _updatePins () {
    ...
    for (let i = 0; i < xLength; i++) {
      for (let j = zLength - 1; j >= 0; j--) {
        box = this._getPin()
        box.isVisible = true
        
        const boxbbInfo = box.getBoundingInfo().boundingBox.extendSize
      
        box.position.y -= (parseFloat(this.metadata['dimensions']['maximum_height']) / 2 + boxbbInfo.y)
        let offsetX = 0
    ...
    this.pins[parseInt(this.metadata['dimensions']['direction'])].position.y += 0.2
  ...
  _createBody3D () {
    ...
    const cylinder = BABYLON.MeshBuilder.CreateCylinder('cylinder1', options, this.scene)
    ...
    let sphere = BABYLON.MeshBuilder.CreateSphere('sphere', options2, this.scene)
    sphere.position.y = (parseFloat(this.metadata['dimensions']['maximum_height']) - 1) / 2

    const cil = cylinder.clone()
    sphere = Component._createHoles(sphere, cil, this.scene)
