...
export default function (scene, metadata) {
  ...
  this._initDPDT = function () {
    ...
    this.object3D = this._createDPDT3D()
  ...
  this._createDPDT3D = function () {
    ...
    this._addPinsDPDTComp(box)

    box.material = bodymaterial

    this._createDPDT2D(box)

    // create an empty mesh to hold the entire components
    var mesh = new BABYLON.Mesh('mesh', this.scene)
    mesh.name = 'DPDT' // this.metadata['name'] + this.metadata['part'] + this.metadata['id']
    box.setParent(mesh)

    return mesh
  }

  //  ADD PINS DPDT COMPONENT
  this._addPinsDPDTComp = function (comp) {
    comp.pins = []

    if (parseInt(this.metadata['package_pin']['e_number_pins']) * parseInt(this.metadata['package_pin']['d_number_pins']) > 6) {
      alert('This package should have max 6 pins')
    }
    let box
  ...
  this._sortPins = function (data) {
    ...
    if (this.metadata.isClockwise) {
      ...
      // sortPins = data
      sortPins = [data[0], data[3], data[1], data[4], data[2], data[5]]
    }
    else {
      /*let auxPinsArray = []
      for (let i = 0; i < data.length; i++) {
      ...
      sortPins = [data[2], data[5], data[1], data[4], data[0], data[3]]
    }
    return sortPins
  }
  
  this._createDPDT2D = function (comp) {
    let meshes = []
