...
export default function (scene, metadata) {
  ...
  this._initSTTF003A = function () {
    //  add Marian - need to come from formSTTF003A
    ...
    this.object3D = this._createSTTF003A3D()
  ...
  //  CREATE 3D STTF003A OBJECT
  this._createSTTF003A3D = function () {
    ...
    this._addPinsSTTF003AComp(box)

    box.material = bodymaterial

    this._createSTTF003A2D(box)
    ...
    mesh.name = 'STTF003A' // this.metadata['name'] + this.metadata['part'] + this.metadata['id']
    box.setParent(mesh)
  ...
  this._createBody = function () {
    ...
    box2.position = new BABYLON.Vector3(-1.5, 0, -2)
    this.metadata['externalObject']['position'] = box2.position
    box = Component._createHoles(box, box2, this.scene)
    return box
  }

  //  ADD PINS STTF003A COMPONENT
  this._addPinsSTTF003AComp = function (comp) {
    comp.pins = []
    ...
    const arrayPos = [[-6.58, 4.73, 1.4, 1.6], [-6.58, -4.77, 1.4, 2.2], [8.22, 4.83, 1.8, 1.4], [9.12, -4.77, 1.4, 2.2]]
    for (let i = 0; i < 4; i++) {
      const box1 = Component._create2DShape(0, arrayPos[i][2], arrayPos[i][3], 0.1, this.scene)

      box1.metadata = {
        'width': arrayPos[i][2],
        'length': arrayPos[i][3]
      }
      box1.position.y = -bbInfo.y
      box1.position.x = arrayPos[i][0]
      box1.position.z = arrayPos[i][1]

      box1.material = clone.material.clone('pinMaterial_' + i)
      comp.pins.push(box1)
    }

    for (let i = 0; i < 2; i++) {
      const box1 = Component._create2DShape(1, 0.45, 0.45, 0.2, this.scene)

      box1.position.y = -(bbInfo.y + 0.1)
      box1.position.z = -5.17

      if (i === 0) {
        box1.position.x = -3.68
      }
      else {
        box1.position.x = 4.32
      }
      ...
      box1.material = new BABYLON.StandardMaterial('pinMaterial', this.scene)
      comp.pins.push(box1)
    }
  ...
  //  CREATE 2D STTF003A OBJECT
  this._createSTTF003A2D = function (comp) {
    let meshes = []
  ...
  this.sendData = function () {
    ...
    const diff = new BABYLON.Vector3(parseFloat(Number(pins[10].getAbsolutePosition().x + x - pins[10].metadata.Fwidth / 2).toFixed(4)), 0, parseFloat(Number(pins[10].getAbsolutePosition().z + z - pins[10].metadata.Flength / 2).toFixed(4)))
    ...
    if (this.scaleFromLayer1) {
      ...
      for (let i = 0; i < pins.length; i++) {
        ...
        if (pins[i].margin) {
          k++
          arrayPins.push([0, parseFloat(Number(pins[i].position.x + this.offsetX + getMax[0]).toFixed(4)), parseFloat(Number(pins[i].position.z + this.offsetZ + getMax[1]).toFixed(4)), arrayPos[k][2], arrayPos[k][3], 0])
        }
        if (pins[i].pins) {
