...
export default class ESP826612E {
  ...
  async _load3dObjects () {
    ////// body ////////
    const body = this._createBody3D() // await Component.import3dObject('user-4f84f14449114c36941b78c058d3adc3.glb', this.scene)
    // const antena = await Component.import3dObject('user-b7207b0e0f4f4024a44316a85a89a6c2.glb', this.scene)
    // const body_top = await Component.import3dObject('user-096b86f13a544e6f8da1e5676c096945.glb', this.scene)
    if (!body) {
      console.log('Error on import body parts')
    }

    body.name = body.id = 'assets'
    // antena.name = antena.id = 'assets'
    // body_top.name = body_top.id = 'assets'

    body.isVisible = false
    // antena.isVisible = false
    // body_top.isVisible = false

    body.rotationQuaternion = null
    // antena.rotationQuaternion = null
    // body_top.rotationQuaternion = null
    
    if (body.material) {
      body.material.dispose()
    }
    /*if (antena.material) {
      antena.material.dispose()
    }
    if (body_top.material) {
      body_top.material.dispose()
    }*/

    this.assets.push(body) //, antena, body_top)
    ////// body ////////

    ////// pins ////////
    const pin_left = this._getPin() // await Component.import3dObject('user-98888b3872e24df7bdb3f4190f04d169.glb', this.scene)
    // const pin_bottom = await Component.import3dObject('user-aeb9796a6cd0441cb82a071e7a09b5a7.glb', this.scene)
    if (!pin_left) {
      console.log('Error on import pin')
    }

    pin_left.name = pin_left.id = 'assets'
    // pin_bottom.name = pin_bottom.id = 'assets'

    pin_left.isVisible = false
    // pin_bottom.isVisible = false

    pin_left.rotationQuaternion = null
    // pin_bottom.rotationQuaternion = null

    if (pin_left.material) {
      pin_left.material.dispose()
    }

    /*if (pin_bottom.material) {
      pin_bottom.material.dispose()
    }*/

    this.assets.push(pin_left) //, pin_bottom)
  }
  ...
  _updateBody () { 
    ...
    // w h d
    const width = parseFloat(this.metadata['dimensions']['d_max_lead_span'])
    const height = parseFloat(this.metadata['dimensions']['maximum_height'])
    const depth = parseFloat(this.metadata['dimensions']['e_max_lead_span'])
  ...
  _updatePins () {
    ...
    const width = parseFloat(this.metadata['dimensions']['d_max_lead_span']) / 2
    const depth = parseFloat(this.metadata['dimensions']['e_max_lead_span']) / 2


    const nrOfPins1 = 2 * parseInt(this.metadata['package_pin']['number_pins_l'])
    const nrOfPins2 = parseInt(this.metadata['package_pin']['number_pins_r'])
    let box
    for (let i = 0; i < nrOfPins1; i++) {
      box = this._getPin()
      box.isVisible = true

      const boxbbInfo = box.getBoundingInfo().boundingBox.extendSize

      let k = i % parseInt(this.metadata['package_pin']['number_pins_l'])

      box.position.x = (i < parseInt(this.metadata['package_pin']['number_pins_l'])) ? -(width - boxbbInfo.z) : (width - boxbbInfo.z)
      box.position.z = -depth + parseFloat(this.metadata['dimensions']['pin_dist_l']) + k * parseFloat(this.metadata['package_pin']['pitch'])

      box.rotation.y = (i < parseInt(this.metadata['package_pin']['number_pins_l'])) ? -Math.PI / 2 : Math.PI / 2
    ...
    for (let i = 0; i < nrOfPins2; i++) {
      box = this._getPin()
      box.isVisible = true

      const boxbbInfo = box.getBoundingInfo().boundingBox.extendSize

      box.position.z = -(depth - boxbbInfo.z)
      box.position.x = -width + parseFloat(this.metadata['dimensions']['pin_dist_r']) + i * parseFloat(this.metadata['package_pin']['pitch'])
  ...
  _addFSize () {
    ...
    for (let i = 0; i < this.pins.length; i++) {
      this.pins[i].position.y += height
    }
    
    // this.metadata['transform']['position'][1] = height + 0.01
    // console.log(this.layer1.position, this.metadata['transform']['position'][1], this.pins[0].scaling.y, height, parseFloat(this.metadata['dimensions']['min_standoff']))
  }
  ...
  _createBody3D () {
    ...
    for (let i = 0; i < 2 * parseInt(this.metadata['package_pin']['number_pins_l']); i++) {
      ...
      box.position.x = (i < parseInt(this.metadata['package_pin']['number_pins_l'])) ? -(bbInfo.x - boxbbInfo.z) : (bbInfo.x - boxbbInfo.z)
      box.position.z = -bbInfo.z + parseFloat(this.metadata['dimensions']['pin_dist_l']) + k * parseFloat(this.metadata['package_pin']['pitch'])
    ...
    for (let i = 0; i < parseInt(this.metadata['package_pin']['number_pins_r']); i++) {
      box = clone.clone()
      box.isVisible = true

      box.position.z = -(bbInfo.z - boxbbInfo.z)
      box.position.x = -bbInfo.x + parseFloat(this.metadata['dimensions']['pin_dist_r']) + i * parseFloat(this.metadata['package_pin']['pitch'])
  ...
  _getPin (simpePin = false) {
    const depth = parseFloat(this.metadata['package_pin']['max_lead_length'])
    const width = parseFloat(this.metadata['package_pin']['max_lead_width'])

    let box = Component._create2DShape(0, width, depth, parseFloat(this.metadata['dimensions']['maximum_height']), this.scene)
    
    if (simpePin) {
      return box
    }

    if (parseFloat(this.metadata['package_pin']['hole_d']) !== 0) {
      const hole = BABYLON.MeshBuilder.CreateCylinder('pin', { height: parseFloat(this.metadata['dimensions']['maximum_height']), diameter: parseFloat(this.metadata['package_pin']['hole_d']), tesselation: 32 }, this.scene)
      hole.position.z = parseFloat(this.metadata['package_pin']['max_lead_length']) / 2
      box = Component._createHoles(box, hole, this.scene)

      const hole2 = BABYLON.MeshBuilder.CreateCylinder('pin', { height: parseFloat(this.metadata['dimensions']['maximum_height']) + 0.1, diameter: parseFloat(this.metadata['package_pin']['hole_d']), tesselation: 32 }, this.scene)
      hole2.position.z = -parseFloat(this.metadata['package_pin']['max_lead_length']) / 5
      box = Component._createHoles(box, hole2, this.scene)
    }
