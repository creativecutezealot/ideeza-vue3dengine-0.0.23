  ...
  _updatePins () {
    ...
    for (let i = 0; i < xLength; i++) {
      for (let j = 0; j < zLength; j++) {
        ...
        let offsetX = 0
        if (zLength % 2 === 0) {
          offsetX = (parseFloat(this.metadata['package_pin']['e1_pitch']) / 2) * (xLength - 1)
        }
        else {
          offsetX = parseFloat(this.metadata['package_pin']['e1_pitch']) * ((xLength - 1) / 2)
        }
        let offsetY = 0
        if (xLength % 2 === 0) {
          offsetY = parseFloat(this.metadata['package_pin']['e_pitch']) / 2 * (zLength - 1)
        }
        else {
          offsetY = parseFloat(this.metadata['package_pin']['e_pitch']) * ((zLength - 1) / 2)
        }
        ...
        box.position.z = depth - parseFloat(this.metadata['package_pin']['pins_pos'])
