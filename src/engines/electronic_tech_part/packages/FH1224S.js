...
export default class FH1224S {
  ...
  _updatePins () {
    ...
    for (let i = 0; i < totalPins; i++) {
      ...
      box.position.y = -(height + parseFloat(this.metadata['dimensions']['min_standoff']))
      box.position.z = -depth

      if (totalPins % 2 !== 0) {
        box.position.x = (i % totalPins) * parseFloat(this.metadata['package_pin']['pitch']) - parseFloat(this.metadata['package_pin']['pitch']) * ((totalPins / 2 < 1) ? 0 : parseInt(totalPins / 2))
      }
      else {
        box.position.x = (i % totalPins) * parseFloat(this.metadata['package_pin']['pitch']) + parseFloat(this.metadata['package_pin']['pitch']) / 2 - parseFloat(this.metadata['package_pin']['pitch']) * (totalPins / 2)
      }
    ...
    for (let i = 0; i < 2; i++) {
      const box = Component._create2DShape(0, 1.8, 2.2, 0.1, this.scene)

      box.position.y = -(height + 0.1)
      box.position.z = 0.75 // 3.25
  ...
  _createLayer21 () {
    ...
    const offsetZ = -parseFloat(this.metadata['package_pin']['max_lead_length']) / 3
    let silks = []
    const path = [[new BABYLON.Vector3(-x, 0, z + offsetZ), new BABYLON.Vector3(-x, 0, -z + offsetZ)],
      [new BABYLON.Vector3(-x, 0, -z + offsetZ), new BABYLON.Vector3(x, 0, -z + offsetZ)],
      [new BABYLON.Vector3(x, 0, -z + offsetZ), new BABYLON.Vector3(x, 0, z + offsetZ)],
      [new BABYLON.Vector3(x, 0, z + offsetZ), new BABYLON.Vector3(-x, 0, z + offsetZ)]]
  ...
  _getElectronicData () {
    ...
    if (this.startedPin) {
      diff = new BABYLON.Vector3(0, 0, - this.startedPin.metadata.Flength / 3)
    }
  ...
  _addFSize () {
    ...
    const height = parseFloat(this.metadata['dimensions']['maximum_height']) + 0.25 // 0.3 - height of pin + min standford
  ...
  _createBody3D () {
    ...
    // this.metadata['externalObject'].position = new BABYLON.Vector3(0, 0, 1)
