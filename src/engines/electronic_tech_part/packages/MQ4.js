...
export default class MQ4 {
  ...
  _updateBody () {
    ...
    this.name.position.y = height * 0.7 + 0.01
    ...
    this.drill.position.y = height * 0.2
  ...
  _updatePins () {
    ...
    let box
    for (let i = parseInt(this.metadata['package_pin']['e_number_pins']); i > 0; i--) {
      box = this._getPin()
      box.isVisible = true
 
      var boxbbInfo = box.getBoundingInfo().boundingBox.extendSize

      let contor = (i > parseInt(this.metadata['package_pin']['e_number_pins']) / 2) ? (i - 1) : (i - 2)
      // console.log(contor, i)
      const radius = parseFloat(this.metadata['dimensions']['e_max_body_span']) / 2
      box.position.y -= boxbbInfo.y
      box.position.x = radius * Math.cos(contor * parseFloat(this.metadata['package_pin']['e_angle']) * Math.PI / 180)
      box.position.z = radius * Math.sin(contor * parseFloat(this.metadata['package_pin']['e_angle']) * Math.PI / 180) 
  ...
  _createBody3D () {
    var options = {
      diameterTop: parseFloat(this.metadata['dimensions']['d_max_body_span']),
      diameterBottom: parseFloat(this.metadata['dimensions']['d_max_body_span']),
      height: parseFloat(this.metadata['dimensions']['maximum_height']) * 0.2,
      tessellation: 32
    }

    const cylinder = BABYLON.MeshBuilder.CreateCylinder('body', options, this.scene)
    cylinder.position.y = parseFloat(this.metadata['dimensions']['maximum_height']) * 0.1
    
    var options2 = {
      diameterTop: parseFloat(this.metadata['dimensions']['d_max_body_span']) * 0.8,
      diameterBottom: parseFloat(this.metadata['dimensions']['d_max_body_span']) * 0.9,
    ...
    sphere.position.y = parseFloat(this.metadata['dimensions']['maximum_height']) * 0.4
