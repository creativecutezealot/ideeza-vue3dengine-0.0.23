...
export default class TO92 {
  ...
  async _load3dObjects () {
    ////// body ////////
    const body = await Component.import3dObject('user-a8e9c7e07c1b4da694b1a5d60e4e9312.glb', this.scene)
    if (!body) {
  ...
  _updateBody () {
    ...
    const body = this.assets[0].clone('bodyMesh')
    body.isVisible = true
    body.scaling.x = width
    body.scaling.y = height
    body.scaling.z = depth
  ...
  _updatePins () {
    ...
    const pinX = parseFloat(this.metadata['package_pin']['max_width_range'])
    const pinY = parseFloat(this.metadata['package_pin']['max_lead_height'])
    const pinZ = parseFloat(this.metadata['package_pin']['max_lead_range'])
    ...
    for (let i = 0; i < xLength; i++) {
      for (let j = 0; j < zLength; j++) {
        ...
        box.position.x = j * e - offsetY
        box.position.z = depth / 3.75
        
        box.position.y = -(height / 2 + pinY / 2 - 0.1)
  ...
  _addFsize () {
    ...
    if (parseInt(this.metadata['package_pin']['type']) === 0) {
      ...
      const dist = parseFloat(this.metadata['dimensions']['d_max_body_span'])
      this.metadata['transform']['position'][1] = dist / 1.8
      this.metadata['transform']['rotation'][0] = -90

      this.layer1.position.z = dist / 1.8 - 0.01
      this.layer21.position.z = dist / 1.8 - 0.01

      this.layer1.rotation.x = Math.PI / 2
      this.layer21.rotation.x = Math.PI / 2

      const newPos = parseFloat(this.metadata['package_pin']['max_lead_height']) + (parseFloat(this.metadata['dimensions']['maximum_height'])) / 2 + parseFloat(this.metadata['package_pin']['max_lead_range']) / 2 - 0.1
      this.layer1.position.y = -newPos + parseFloat(this.metadata['package_pin']['max_lead_height']) / 2 + parseFloat(this.metadata['dimensions']['maximum_height']) + parseFloat(this.metadata['package_pin']['max_width_range']) + 0.2 // to check
      this.layer21.position.y = -newPos + parseFloat(this.metadata['package_pin']['max_lead_height']) /2 + parseFloat(this.metadata['dimensions']['maximum_height']) + parseFloat(this.metadata['package_pin']['max_width_range']) + 0.2 // to check
