...
export default class SX1278 {
  ...
  _updatePins () {
    ...
    let pinsOn = parseFloat(this.metadata['dimensions']['pin_dist_l'])
    let pinsNr = parseInt(this.metadata['package_pin']['number_pins_l'])

    const width = parseFloat(this.metadata['dimensions']['e_max_lead_span']) / 2
    const depth = 2 * pinsOn + (pinsNr - 1) * parseFloat(this.metadata['package_pin']['pitch'])
    const height = parseFloat(this.metadata['dimensions']['maximum_height']) / 2

    const nrOfPins1 = parseInt(this.metadata['package_pin']['number_pins_l'])
    const nrOfPins2 = parseInt(this.metadata['package_pin']['number_pins_r'])
    let box
    for (let i = 0; i < nrOfPins1; i++) {
      box = this._getPin(false, true)
      box.isVisible = true

      const boxbbInfo = box.getBoundingInfo().boundingBox.extendSize

      box.position.x = -width + parseFloat(this.metadata['package_pin']['max_lead_length']) - boxbbInfo.z / 2 - 0.2
      if (parseInt(this.metadata['package_pin']['number_pins_l']) % 2 !== 0) {
        box.position.z = (i % parseInt(this.metadata['package_pin']['number_pins_l'])) * parseFloat(this.metadata['package_pin']['pitch']) - parseFloat(this.metadata['package_pin']['pitch']) * ((parseInt(this.metadata['package_pin']['number_pins_l']) / 2 < 1) ? 0 : parseInt(parseInt(this.metadata['package_pin']['number_pins_l']) / 2))
      }
      else {
        box.position.z = (i % parseInt(this.metadata['package_pin']['number_pins_l'])) * parseFloat(this.metadata['package_pin']['pitch']) + parseFloat(this.metadata['package_pin']['pitch']) / 2 - parseFloat(this.metadata['package_pin']['pitch']) * (parseInt(this.metadata['package_pin']['number_pins_l']) / 2)
      }
    ...
    for (let i = 0; i < nrOfPins2; i++) {
      ...
      box.position.x = width - boxbbInfo.z
      box.position.y = parseFloat(this.metadata['dimensions']['maximum_height']) / 2
      box.position.z = depth / 2 - parseFloat(this.metadata['dimensions']['pin_dist_r']) - i * parseFloat(this.metadata['package_pin']['pitch'])
      box.rotation.y = Math.PI / 2
  ...
  _getElectronicData () {
    ...
    for (let i = 0; i < pins.length; i++) {
      arrayPins.push([1, parseFloat(Number(pins[i].metadata['position'].x + x - diff.x).toFixed(this.footPrint.resolution)), parseFloat(Number(pins[i].metadata['position'].z + z - diff.z).toFixed(this.footPrint.resolution)), parseFloat(Number(pins[i].metadata.Flength).toFixed(this.footPrint.resolution)), parseFloat(Number(pins[i].metadata.Fwidth).toFixed(this.footPrint.resolution)), 0])
    }
  ...
  _addFSize () {
    ...
    const height = parseFloat(this.metadata['dimensions']['maximum_height']) / 2
    this.layer1.position.y = -3 + 0.01
    this.layer21.position.y = -3 + 0.01 
    ...
    this.metadata['transform']['position'][1] = 3 
  ...
  _createBody3D () {
    let pinsOn = parseFloat(this.metadata['dimensions']['pin_dist_l'])
    let pinsNr = parseInt(this.metadata['package_pin']['number_pins_l'])
    if (parseInt(this.metadata['package_pin']['number_pins_l']) < parseInt(this.metadata['package_pin']['number_pins_r'])) {
      pinsOn = parseFloat(this.metadata['dimensions']['pin_dist_r'])
      pinsNr = parseInt(this.metadata['package_pin']['number_pins_r'])
    }
    const depth = 2 * pinsOn + (pinsNr - 1) * parseFloat(this.metadata['package_pin']['pitch'])
    ...
    for (let i = 0; i < parseInt(this.metadata['package_pin']['number_pins_r']); i++) {
      ...
      box.position.x = bbInfo.x * comp.scaling.x - boxbbInfo.z * box.scaling.z
      box.position.z = bbInfo.z * comp.scaling.z - parseFloat(this.metadata['dimensions']['pin_dist_r']) - i * parseFloat(this.metadata['package_pin']['pitch'])
  ...
  _getPin (simpePin = false, th = false) {
    const depth = parseFloat(this.metadata['package_pin']['max_lead_length'])
    const width = parseFloat(this.metadata['package_pin']['max_lead_width'])

    let box 
    if (simpePin) {
      box = Component._create2DShape(0, width, depth, parseFloat(this.metadata['dimensions']['maximum_height']), this.scene)
      box.isVisible = false
      return box
    }

    if (th) {
      const length = parseFloat(this.metadata['package_pin']['hole_d'])
      const height = 4.8
