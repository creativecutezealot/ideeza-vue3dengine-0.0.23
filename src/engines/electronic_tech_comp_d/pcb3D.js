...
export default {
  ...
  async init (scene, data) {
    ...
    this.data.routing_data = this.nets = data.routing_data
  ...
  async handleImport (engineData, ids) {
    ...
    if (BBJSurl) {
      ...
      if (engineData.transform.best_p[0] === 0 && engineData.transform.best_p[1] === 0 && engineData.transform.best_p[2] === 0) {
        // place them near component
        const x = this.data.compSize[0] / 2 + 5
        const indexOfPart = this.getIndexOfPorp(this.objects[ meshes[0].metadata.id1],  meshes[0].metadata.id)
        const z = this.data.compSize[1] / 2 - indexOfPart * 5
        meshes[0].position = new BABYLON.Vector3(x, 0.01, z)
      ...
      new BABYLON.ExecuteCodeAction(BABYLON.ActionManager.OnPickDownTrigger, async (evt) => {
        ...
        if (evt.sourceEvent.button === 0) {
          switch (_this.curentOpt) {
            ...
            case 1:
              _this.selected = _thism
              console.log((_this.selected.rotation.y + Math.PI / 2) % (2 * Math.PI))
  ...
  getData () {
    ...
    if (this.completlyLoaded) {
      this.data.routing_data = this.nets
  ...
  getCompDimensions () {
    ...
    return [bbInfo.maximumWorld.x, bbInfo.maximumWorld.z]
  },
